...
#
# CYCLE (wipe-and-replace => zero staleness):
...
REPO_DIR="${1:-$PWD}"
cd "$REPO_DIR" 2>/dev/null || { echo "graphify-reseed: cannot cd '$REPO_DIR'"; exit 1; }
...
if command -v graphify >/dev/null 2>&1; then
  graphify update . >/dev/null 2>&1 || echo "graphify-reseed: 'graphify update' failed (continuing with existing report)"
fi

# Nothing to seed if the graph was never built.
[ -f "$REPORT" ] || { echo "graphify-reseed: no $REPORT — skipping"; exit 0; }
...
timeout 600 "$MP" mine "$TMP" --wing "$WING" --no-gitignore --agent graphify-reseed >/dev/null \
  ...
  || { echo "graphify-reseed: mine failed or timed out (lock held?) — wing left wiped, will retry next run"; exit 1; }
