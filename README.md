...
separately-accounted tool-schema deferral or provider prompt-cache discounts.
The workflow doctor reports those layers independently and identifies the live
Headroom mode and profile. When using `headroom agent-savings --check-perf`,
only compare a savings-profile target with traffic generated by that same profile.

Provider prefix caching is provider-owned. Headroom observes the provider's
cache counters and can improve prefix stability, but the reported cache discount
must not be treated as wholly caused by Headroom.
