...
test_instructions() {
  ...
  assert_file_contains "$REPO_DIR/README.md" \
    '`headroom perf` reports direct proxy compression; it does not include' \
    'README separates direct Headroom compression from other savings layers'
  assert_file_contains "$REPO_DIR/README.md" \
    'only compare a savings-profile target with traffic generated by that same profile' \
    'README prevents cross-profile Headroom savings comparisons'
  assert_file_contains "$REPO_DIR/README.md" \
    'Provider prefix caching is provider-owned' \
    'README avoids over-attributing provider cache savings to Headroom'
...
test_doctor() {
  local tmp home codex_dir fake_bin config output log health_fixture stats_fixture
  local headroom_output mismatch_output malformed_output
...
EOF
  health_fixture="$tmp/headroom-health.json"
  stats_fixture="$tmp/headroom-stats.json"
  cat > "$health_fixture" <<'EOF'
{"status":"healthy","config":{"savings_profile":"coding"}}
EOF
  cat > "$stats_fixture" <<'EOF'
{"summary":{"mode":"cache"},"tokens":{"proxy_compression_saved":59636,"proxy_total_before_compression":92375300,"proxy_savings_percent":0.06},"savings":{"by_layer":{"tool_search":{"tokens_saved":301158,"requests":17}}},"prefix_cache":{"totals":{"hit_rate":96.8,"cache_read_tokens":94520190,"net_savings_usd":230.2561}}}
EOF
  cat > "$fake_bin/curl" <<'EOF'
#!/usr/bin/env bash
url="${!#}"
case "$url" in
  */livez|*/readyz) exit 0 ;;
  */health)
    [ "${HEADROOM_DETAIL_STATE:-valid}" != unavailable ] || exit 1
    if [ "${HEADROOM_DETAIL_STATE:-valid}" = malformed ]; then
      printf '{\n'
    else
      cat "$HEADROOM_HEALTH_FIXTURE"
    fi
    ;;
  */stats)
    [ "${HEADROOM_DETAIL_STATE:-valid}" != unavailable ] || exit 1
    cat "$HEADROOM_STATS_FIXTURE"
    ;;
  *) exit 1 ;;
esac
EOF
  for command in headroom mempalace mempalace-mcp graphify; do
...
EOF
  ...
  chmod +x "$fake_bin/codex-explicit" "$fake_bin/docker" "$fake_bin/zsh" "$fake_bin/curl"
  ...
  headroom_output="$(HOME="$home" CODEX_DIR="$codex_dir" CODEX_BIN="$fake_bin/codex-explicit" \
    CODEX_DOCTOR_RUNTIME=1 TEST_LOG="$log" PATH="$fake_bin:/usr/bin:/bin" \
    HEADROOM_HEALTH_FIXTURE="$health_fixture" HEADROOM_STATS_FIXTURE="$stats_fixture" \
    "$REPO_DIR/tools/codex/doctor-workflow.sh" 2>&1)"
  assert_text_contains "$headroom_output" \
    'PASS headroom active policy mode=cache profile=coding' \
    'doctor reports the active Headroom policy'
  assert_text_contains "$headroom_output" \
    'PASS headroom direct compression: 59636/92375300 tokens (0.06%)' \
    'doctor labels direct Headroom compression separately'
  assert_text_contains "$headroom_output" \
    'PASS headroom tool-schema deferral: 301158 tokens across 17 requests' \
    'doctor reports Headroom-attributed tool-schema deferral separately'
  assert_text_contains "$headroom_output" \
    'PASS provider-observed prefix cache: 96.8% token hit rate, 94520190 read tokens, $230.2561 net discount' \
    'doctor labels provider prefix-cache observations separately'

  printf '%s\n' '{"status":"healthy","config":{"savings_profile":"agent-90"}}' > "$health_fixture"
  printf '%s\n' '{"summary":{"mode":"token"}}' > "$stats_fixture"
  mismatch_output="$(HOME="$home" CODEX_DIR="$codex_dir" CODEX_BIN="$fake_bin/codex-explicit" \
    CODEX_DOCTOR_RUNTIME=1 TEST_LOG="$log" PATH="$fake_bin:/usr/bin:/bin" \
    HEADROOM_HEALTH_FIXTURE="$health_fixture" HEADROOM_STATS_FIXTURE="$stats_fixture" \
    "$REPO_DIR/tools/codex/doctor-workflow.sh" 2>&1)"
  assert_text_contains "$mismatch_output" \
    'WARN headroom policy differs from repository-managed mode=cache profile=coding (observed mode=token profile=agent-90)' \
    'doctor warns when measured traffic uses a different Headroom policy'

  malformed_output="$(HOME="$home" CODEX_DIR="$codex_dir" CODEX_BIN="$fake_bin/codex-explicit" \
    CODEX_DOCTOR_RUNTIME=1 TEST_LOG="$log" PATH="$fake_bin:/usr/bin:/bin" \
    HEADROOM_DETAIL_STATE=malformed HEADROOM_HEALTH_FIXTURE="$health_fixture" \
    HEADROOM_STATS_FIXTURE="$stats_fixture" \
    "$REPO_DIR/tools/codex/doctor-workflow.sh" 2>&1)"
  assert_text_contains "$malformed_output" \
    'PASS headroom proxy live and ready on 127.0.0.1:8787' \
    'doctor preserves Headroom liveness when detail JSON is malformed'
  assert_text_contains "$malformed_output" \
    'WARN headroom optimization details unavailable' \
    'doctor warns when Headroom detail JSON cannot be parsed'
