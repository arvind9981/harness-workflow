#!/usr/bin/env bash
# MemPalace health worker (runs DETACHED, never blocks a session).
# Launched fire-and-forget by mempalace-health.sh on SessionStart. Detects the
# three failure modes a crashed/SIGKILL'd writer leaves behind:
#   1. stale mine_palace lock (dead owner PID) -> removes it
#   2. malformed FTS5 inverted index           -> logs offline recovery
#   3. corrupt HNSW index (query DEADLOCKS)     -> logs offline recovery
...
if [ "$qc" != "ok" ]; then
  ...
  if printf '%s' "$qc" | grep -qi 'fulltext\|fts'; then
    log "FTS5 corruption detected; stop mempalace-mcp before any manual rebuild"
  fi
fi
...
log "HNSW probe FAILED (rc=$rc; 124=deadlock/timeout); stop mempalace-mcp before manual repair --mode from-sqlite"
exit 0
